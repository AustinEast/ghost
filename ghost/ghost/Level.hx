 ...
 */
class Level implements IDisposable implements IProxy {
      ...
      if (game.project != null) {
        ...
        if (tileset != null) {
          tilemap.load_from_array(tile_data, layer_definition.gridCellsX, layer_definition.gridCellsY, {
            ...
            tile_graphic: hxd.Res.load(tileset.path).toTile()
          });
        }
      }
    }
      ...
      if (game.project != null) {
        ...
        if (layer_template != null && layer_template.legend != null) {
          for (key => value in layer_template.legend) {
            grid.legend.set(key, ghost.util.Color.fromString(value, true));
          }
        }
      }
          ...
          if (game.project != null) {
            ...
            if (template != null) {
              if (template.texture != null) {
                e.renderer.load(hxd.Res.load(template.texture).toImage());
                if (entity.height != null) {
                  e.renderer.scale_y = entity.height / template.size.y;
                }
              }
              else {
                var color = ghost.util.Color.fromString(template.color, true);
    ...
    data.onDecalLayerLoaded = (decals, layer_definition) -> {
      ...
      for (decal in decals) {
        var d = new Sprite({
          ...
          renderer: {
            asset: hxd.Res.load(layer_definition.folder + '/' + decal.texture).toImage(),
