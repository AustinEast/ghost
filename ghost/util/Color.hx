...
/**
 *  TODO - replace with ghost-specific color class
 *
...
abstract Color(Int) from Int from UInt to Int to UInt {
   ...
   */
  public static function fromString(str:String, EndsWithAlpha:Bool = false):Null<Color> {
    var result:Null<Color> = null;
    str = StringTools.trim(str);

    if (COLOR_REGEX.match(str)) {
      var colorInts = COLOR_REGEX.matched(2);
      if (EndsWithAlpha && colorInts.length == 8) colorInts = colorInts.substr(6, 2) + colorInts.substr(0, 6);
      var hexColor:String = "0x" + colorInts;
  ...
  // 	var warmer:Color = fromHSB(FlxMath.wrap(oppositeHue - Threshold, 0, 350), saturation, brightness, alphaFloat);
  // 	var colder:Color = fromHSB(FlxMath.wrap(oppositeHue + Threshold, 0, 350), saturation, brightness, alphaFloat);
  // 	return {original: this, warmer: warmer, colder: colder};
  // }
  /**
  ...
  // {
  // 	var triadic1:Color = fromHSB(FlxMath.wrap(Std.int(hue) + 120, 0, 359), saturation, brightness, alphaFloat);
  // 	var triadic2:Color = fromHSB(FlxMath.wrap(Std.int(triadic1.hue) + 120, 0, 359), saturation, brightness, alphaFloat);
  ...
  public inline function toHexString(Alpha:Bool = true, Prefix:Bool = true):String {
    return (Prefix ? "0x" : "")
      + (Alpha ? StringTools.hex(alpha, 2) : "")
      + StringTools.hex(red, 2)
      + StringTools.hex(green, 2)
      + StringTools.hex(blue, 2);
  }
  /**
