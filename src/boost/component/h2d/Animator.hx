...
class Animator extends Component {
	...
	@:allow(boost.system.h2d.Animations)
	var delay:Float;
	@:allow(boost.system.h2d.Animations)
	var timer:Float;
	@:allow(boost.system.h2d.Animations)
	...
	public function add(name:String, frames:Array<Int>, speed:Int = 15, looped:Bool = false, loop_delay:Float = 0, direction:AnimationDirection = AnimationDirection.FORWARD):Animation {
		members.exists(name) ? GM.log.warn('Animation `${name}` already exists on this Entity') 
		: members.set(name, { name:name, frames:frames, speed:speed, looped:looped, loop_delay:loop_delay, direction:direction });
