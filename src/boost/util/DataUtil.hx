...
/**
 * Class to provide different Utilies for dealing with Object Data
 */
class DataUtil {
  /**
  ...
  public static function copy_fields(from:Dynamic, into:Dynamic):Dynamic {
    ...
    if (into == null) into = Reflect.copy(from); else for (f in Reflect.fields(from)) Reflect.setField(into, f, Reflect.field(from, f));
  ...
  public static function copy_fields_c(from:Dynamic, ?into:Dynamic):Dynamic {
    ...
    if (from != null) for (f in Reflect.fields(from)) {
      ...
      if (f.indexOf("color") == 0) {
        if (Std.is(d, String)) {
          Reflect.setField(into, f, Color.fromString(d));
          continue;
        }
      }
